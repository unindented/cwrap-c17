...
`.clang-format` owns mechanical code layout and sets `ReflowComments: false`. `cwrap` owns comment wrap. Do not restate whitespace, wrapping, indentation, brace-placement, or alignment rules here unless they affect meaning.
...
- Name include guards `CWRAP_<FILE>_H`, such as `CWRAP_CMD_WRAP_H` in `cmd_wrap.h`.
...
- Name every exported function `<module>_<verb>`, where the module is the source file's base name: `comment.c` defines `comment_group`, and `fs.c` defines `fs_read_file`.
...
- When a function breaks an operation into steps and delegates each step to a lower-level helper that only it calls, prefix each such helper with the higher-level function's name to make the delegation chain visible (`fs_read_file` calls `fs_read_file_bytes`). This applies only to step helpers dedicated to a single caller. A generic helper reused by several callers, or a self-contained routine that merely happens to have one caller today, takes a plain readable name instead.
- The wrap pipeline needs no exception to the step-helper rule. `rewrite_source` delegates each stage to an exported function of another module (`lex_comment_spans`, `comment_group`, `doxygen_align_parameters`, `fill_body_lines`, `comment_emit`), so each already carries its own module prefix and none is a private step helper. [ARCHITECTURE.md](ARCHITECTURE.md) documents the stages under those names.
...
- Use `snake_case` for functions, variables, fields, directories, and file-local helpers. Source files are named after the module or primary type (`comment.c`), and live in the responsibility directory documented in [ARCHITECTURE.md](ARCHITECTURE.md).
- Use `PascalCase` for `struct` and `enum` tags, such as `CommentBlock`.
- Use `PascalCase` with an `Fn` suffix for callback typedefs. The tree carries no callback typedef yet; the rule stands for the first one.
- Use `UPPER_SNAKE` for `enum` constants, prefixed by the concept they belong to, such as `CLI_ACTION_HELP` and `BODY_LINE_PROSE`.
- Use `UPPER_SNAKE` for a file-local constant that names a fixed literal value: a scalar, a fixed string, or a small list of default literals that stands in for a magic value. The tree's instances are all scalars (`ARENA_CHUNK_CAPACITY_MIN`, `ERROR_MESSAGE_SIZE`, `FS_REASON_SIZE`). Declare a constant string pointer as `static const char* const NAME` so neither the pointer nor the text can be rebound. Keep `snake_case` for read-only aggregate data that reads as a data structure rather than a named literal, such as a lookup or dispatch table of specs or a singleton struct instance; the tree has no such table yet.
- Treat acronyms as words: `CliOptions` in `PascalCase`, `cli_parse` in `snake_case`.
...
- Keep diagnostics lowercase and without trailing periods. A proper noun keeps its capital wherever it falls, including at the start (`C source exceeds max readable size (...)`), per the naming rule in [Comments](#comments).
...
- Quote literal names and user-visible values with single quotes, including paths, filenames, option names, and environment variables.
- Lead with the failed operation, then the relevant key when that value is bounded, then the cause, such as `option '--width' must be a positive integer`. An unbounded value goes last instead, after the cause. That covers an input path, an output path, and a raw argument, so a pathological value truncates itself rather than the reason it was meant to explain. A filesystem path is never treated as bounded, even when every current caller passes a literal: name the subject in prose and let the path trail, as in `failed to read file: No such file or directory ('src/domain/lex.c')`. What a caller happens to pass today is not a bound the message can rely on.
...
- Trail a value in one of three shapes, chosen by the value's role, and do not mix them. Use `: '<value>'` for the value that broke the rule, so the message reads as a judgment of it (`option '--width' must be a positive integer: '0'`). Use `('<path>')` for the path an operation targeted when the cause came from somewhere else, such as `errno` or a callee (`failed to read file: No such file or directory ('src/domain/lex.c')`). Use `(in '<file>')` for the file a failure was located inside; no diagnostic here takes that shape yet, since a rewrap failure names the file it was reading. Where a value fills some other role, name that role in words rather than reaching for a bare parenthesis: `(for '<source>', to '<destination>')` when there are two. A value that reads as part of the sentence stays in the sentence (`option '--width' must be a positive integer`). These shapes govern only a value appended after the cause.
...
- Capitalize language and format names wherever they are prose, including inside a diagnostic (`failed to parse C`, `invalid Doxygen tag`). A name is a name, and lowercasing it makes it harder to recognize and to search for. Identifiers, filenames, extensions, and a library's own spelling stay lowercase because there the text is the token rather than the name (`rewrite_source`, `src/domain/doxygen.c`, `.c`, `copt`).
- Quote literal paths, filenames, directory names, and option names with backticks, the same as code identifiers in Doxygen blocks.
...
- Open the block with an `@brief` line stating what the function does in the third-person present ("Rewrites wrapable comments.", "Appends a single byte."). Keep the voice consistent across the codebase.
...
- Order tests as described in [Readability](#readability), and comment each with what it establishes rather than a restatement of its name. Each test case carries a `//` comment. Every helper in `tests/` carries a Doxygen block and follows [Function documentation](#function-documentation).
