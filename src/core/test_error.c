...
// `expected_errno_reason` in `test_fs.c` derives its expectation from `strerror`, so every `fs`
// reason assertion also pins `error_system_message`'s success branch against the running libc, for
// `ENOENT` and `EISDIR`. The `strerror_r`-failed branch is reachable only from an `errno` the
...
static void test_report_error_formats_and_returns_failure(void) {
  char err[64];
  TEST_CHECK(error_report(err, sizeof(err), "failed to read file: %s ('%s')", "no such file",
                          "input.c") == -1);
  TEST_CHECK(strcmp(err, "failed to read file: no such file ('input.c')") == 0);
}
