...
// remaining space. Large reconstructed comment bodies can exceed `ARENA_CHUNK_CAPACITY_MIN` and
// `arena_strndup` stores them among smaller allocations from the same arena. Losing this behavior
// would cost a `malloc` and most of a chunk each time that happens.
