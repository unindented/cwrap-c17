...
// `string_buffer_append_len`'s length guard exists for rather than for speed. Wrapping helpers may
// forward zero-length chunks, while this test directly covers the documented `NULL` form. Without
// the guard the call reaches `memcpy(dst, NULL, 0)`, which is undefined even for a zero length and
// which no sanitizer here reports.
...
// than a lower bound. Content surviving is true under any growth factor, so without the exact
// assertion neither the seed nor the doubling is pinned and a buffer that grew one byte at a time
// would pass.
