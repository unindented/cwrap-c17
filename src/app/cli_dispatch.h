#ifndef CWRAP_CLI_DISPATCH_H
#define CWRAP_CLI_DISPATCH_H

#include "app/exit_code.h"

/**
 * @brief Parses the command line, then runs the requested action or wrap command.
 *
 ...
 *         `EXIT_CODE_FAILURE` when wrapping fails or the version or help text cannot be written.
 *         Does not return when the parse result is internally inconsistent.
 */
