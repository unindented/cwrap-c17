...
archive_name="cwrap-$version-$target"
stage_dir=$(mktemp -d "${TMPDIR:-/tmp}/cwrap-release.XXXXXX")
...
make -C "$repo_root" release CC="$cc" BUILD_DIR="$build_dir" OUT="$install_dir/cwrap" VERSION="$version"
